...
- `survey_parser` cognitive_after_driving: 실제 스키마 샘플로 확인 — `cognitive_before_driving`과 동일 구조(`payload.cognitive_task_results[]`), `post_nback1` 등으로 정확히 명명됨
...
1. **watch 폴더 실제 파일/컬럼명**: `watch_hr.csv` 등의 정확한 컬럼명(예: `timestamp` vs `t_sec`)을 실측 샘플로 확인 못함 — 현재 `_filter_csv_by_time`이 `timestamp`/`t_sec` 둘 다 시도하도록 방어적으로 짜뒀지만, 실제 컬럼명이 다르면 조용히 빈 파일이 나올 수 있음 (경고 로그 추가 권장)
...
3. **복합 동작 분할 개수 제한**: 현재 제한 없음(라벨러 자유). 제한이 필요하면 `LabelDraft` 생성 시점에 검증 로직 추가
4. **레이더 seg 폴더 간 시간 역전 가능성**: `RadarTimestampIndex`는 안전하게 `ros_time_sec` 기준으로 전체 재정렬하지만, 만약 서로 다른 seg의 프레임이 실제로 뒤섞여 저장되는 경우가 있다면(정상적으론 없어야 함) 프레임 하나하나가 별도 파일 I/O를 일으켜 느려질 수 있음 — 필요시 연속 구간 배치 read로 최적화 가능
...
- **`widget.grab()`으로 실제 화면 스크린샷 저장**: offscreen 플랫폼도 실제로 픽셀을 렌더링하므로, 5개 페이지(시작/시나리오선택/distraction/drowsiness/cognitive 라벨링)를 PNG로 저장해서 실제 레이아웃 확인. 이 과정에서 **`TimelineWidget`의 좌표 계산이 시간 범위를 벗어난 마커에서 정수 오버플로우로 세그폴트를 일으키는 버그를 발견해 수정**함 (`_ts_to_x()`에 안전 범위 clamp 추가, `labeling_page.py`의 전체 타임라인 범위 계산에 task window 시각도 포함하도록 개선)
- **PulseAudio 더미 싱크(`module-null-sink`)로 실제 오디오 재생 테스트**: `QMediaPlayer`를 재생시키고 실제 이벤트 루프를 2.5초 돌려서 `positionChanged`가 시간에 따라 진짜로 움직이는 것 확인 (0→64→192→320ms). 오디오 동기화 메커니즘 자체는 실증됨.
- 다만 테스트에 쓴 wav가 비정상적인 샘플레이트(1000Hz)의 합성 오디오라 재생이 중간에 멈췄음 — **실제 배포 머신에서 정상 샘플레이트(44.1k/48kHz) 마이크 녹음으로 재생 시 끝까지 잘 흘러가는지, 영상이 오디오와 함께 움직이는지 재확인 권장**
