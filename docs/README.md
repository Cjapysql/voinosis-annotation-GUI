...
- `back_*.md`, `front_*.md`, `entry_points.md` — 파일별 상세 문서 (총 22개).
  ...
  메서드의 동작 원리, 그리고 관련된 `design_considerations.md`/
  `session_issues_log.md` 절 번호로의 링크를 포함한다.
...
작업 안에서 이 폴더의 대응 문서도 갱신한다. 설계 결정 자체가 바뀌었다면
`design_considerations.md`에도 새 절을 추가하고, 그 과정에서 버그를 발견/수정한
경우라면 `session_issues_log.md`에도 절을 추가한 뒤 이 폴더의 문서에서
번호로 링크한다.
